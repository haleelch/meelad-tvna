...
(function () {
  ...
  function ensureStateDefaults() {
    ...
    if (state.limits.maxStage == null) state.limits.maxStage = 99;
    if (state.limits.maxNonStage == null) state.limits.maxNonStage = 99;
  ...
  //
  // Individual Top Score: sum of RANK_POINTS (5/3/1) across every Individual
  // programme this student placed 1st/2nd/3rd in \u2014 same rank-based points
  // used everywhere else (team leaderboard, student profile), no grading.
  function computeStudentTotalMarks(studentId, stageTypeFilter) {
    let total = 0;
    Object.entries(state.results).forEach(([eventId, r]) => {
      const event = state.events.find((e) => e.id === eventId);
      if (!event || event.type === "Group") return; // Group programme points count toward the team only, not individual Top Score
  ...
  function renderResultTable(eventId) {
    ...
    return `
      ...
      <table class="result-detail-table"><thead><tr>
        <th>No</th><th>Chest No</th><th>Name</th><th>Code Letter</th><th>Team</th><th>Mark</th>
      </tr></thead><tbody>
        ${ranked.map((r, i) => {
  ...
  // ONLY. Team Points is an internal scoring detail and is shown to admins
  // only (Admin -> Result -> View), never on the public home page feed or
  // the search-icon result block.
  ...
  function renderLimitsTab() {
    ...
    adminContent.innerHTML = `
      <div class="card">
        ...
        <div class="grid2" style="margin-bottom:.75rem">
          <div><div class="field-label">Max Stage Events</div><input id="limStage" type="number" min="0" class="input" value="${state.limits.maxStage}" /></div>
          <div><div class="field-label">Max Non-Stage Events</div><input id="limNonStage" type="number" min="0" class="input" value="${state.limits.maxNonStage}" /></div>
        </div>
        <div class="muted" style="font-size:.72rem;margin-bottom:.75rem">A student must fit under BOTH limits that apply to a programme \u2014 e.g. an Individual + Stage item counts against both the Individual cap and the Stage cap.</div>
    ...
    document.getElementById("btnSaveLimits").addEventListener("click", () => {
      state.limits = {
        ...
        maxNonStage: Number(document.getElementById("limNonStage").value) || 0,
      };
  ...
  function renderStudentsTab() {
    ...
    const selNonStage = studentForm.events.filter((id) => state.events.find((e) => e.id === id)?.stageType === "Off-stage").length;
    ...
    adminContent.innerHTML = `
      <div class="card">
        ...
        <div class="row-between" style="margin-bottom:.4rem">
          ...
          <span class="muted" style="font-size:.65rem;font-family:'JetBrains Mono',monospace">${selIndiv}/${state.limits.maxIndividual} indiv \u00b7 ${selGroup}/${state.limits.maxGroup} group \u00b7 ${selStage}/${state.limits.maxStage} stage \u00b7 ${selNonStage}/${state.limits.maxNonStage} non-stage</span>
        </div>
          ...
          ${eligible.length === 0 ? `<div class="muted" style="font-size:.72rem;font-style:italic">No programmes match this category/gender.</div>` :
            eligible.map((ev) => {
              ...
              const disabled = !checked && (
                (ev.type === "Individual" && selIndiv >= state.limits.maxIndividual) ||
                (ev.type === "Group" && (selGroup >= state.limits.maxGroup || groupTeamFull(ev.id, studentForm.team))) ||
                (stageType === "Stage" && selStage >= state.limits.maxStage) ||
                (stageType === "Off-stage" && selNonStage >= state.limits.maxNonStage)
              );
    ...
    document.querySelectorAll("#checkList input[type=checkbox]").forEach((cb) => {
      cb.addEventListener("change", () => {
        ...
        if (cb.checked) {
          ...
          const stageLimit = stageType === "Stage" ? state.limits.maxStage : state.limits.maxNonStage;
          if (stageCount >= stageLimit) { cb.checked = false; showToast(`Maximum ${stageType.toLowerCase()} event limit reached for this student`); return; }
  ...
  function renderStudentProfile(studentId) {
    ...
    const selNonStage = student.events.filter((id) => state.events.find((e) => e.id === id)?.stageType === "Off-stage").length;
        ...
        <div class="checkbox-list" id="pCheckList">
          ${eligible.length === 0 ? `<div class="muted" style="font-size:.72rem;font-style:italic">No programmes match this category/gender.</div>` :
            eligible.map((ev) => {
              ...
              const disabled = !checked && (
                (ev.type === "Individual" && selIndiv >= state.limits.maxIndividual) ||
                (ev.type === "Group" && (selGroup >= state.limits.maxGroup || groupTeamFull(ev.id, student.team, student.id))) ||
                (stageType === "Stage" && selStage >= state.limits.maxStage) ||
                (stageType === "Off-stage" && selNonStage >= state.limits.maxNonStage)
              );
  ...
  // Guests never see this: Prize / Points are admin-only scoring detail.
      ...
      body = `
        ...
        <table><thead><tr><th>Standing</th><th>Chest No</th><th>Candidate Name</th><th>Team</th><th>Points</th></tr></thead><tbody>
          ${list.map((r, i) => {
            ...
            return `<tr><td>${ORDINAL(i + 1)}</td><td>${r.student.chestNo}</td><td>${escapeHtml(r.student.name)}</td><td>${team ? escapeHtml(team.name) : ""}</td><td>${points ?? "-"}</td></tr>`;
