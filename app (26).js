...
(function () {
  ...
  function ensureStateDefaults() {
    ...
    state.prizeGiven = state.prizeGiven || {}; // { eventId: true } — one tick per programme, not per winner
  ...
  function cleanOrphanedData() {
    ...
    Object.keys(state.prizeGiven || {}).forEach((eventId) => {
      if (!validEventIds.has(eventId)) delete state.prizeGiven[eventId];
    });
    return removed;
  }
  ...
  function renderResultTable(eventId) {
    ...
    return `
      ...
      <table class="result-detail-table"><thead><tr>
        <th>No</th><th>Chest No</th><th>Name</th><th>Code Letter</th><th>Team</th><th>Points</th>
      </tr></thead><tbody>
        ${ranked.map((r, i) => {
  ...
  function drawMasterCard(student) {
    ...
    return templatePromise.then((img) => {
      ...
      //
      // The red box's own text is still vertically centred on itself whether
      // it's 1 word ("HALEEL") or 2 words ("MUHAMMED HAKLEEL"). But
      // nameBottom \u2014 where the Chest No / Category / Team block below
      // starts \u2014 always reserves space as if the name were NAME_RESERVED_LINES
      // tall, so that block stays put at a fixed spot instead of creeping up
      // whenever a name happens to fit on a single line.
      const NAME_RESERVED_LINES = 2;
      const namePlacement = placements.find((p) => p.tag === "student_name");
      let nameBottom = CH * 0.4;
      let nameCenterX = CW - PAD_RIGHT; // fallback if there's somehow no name placement
      if (namePlacement) {
        ...
        nameBottom = ny + (Math.max(NAME_RESERVED_LINES, nameVal ? nameVal.split(" ").length : 0) * lineHeight) / 2;
        if (nameVal) {
      ...
      // Chest No / Category / Team stack right below the name, centred on the
      // same x position as the name itself (the red box), not a separate
      // right-padding anchor \u2014 keeps every line lined up under the name.
  ...
  function renderAdminTab(tab) {
    ...
    if (tab === "prize") return renderPrizeTab();
  ...
  function renderLimitsTab() {
    ...
    adminContent.innerHTML = `
      <div class="card">
        ...
        <div class="muted" style="font-size:.7rem;margin-bottom:.5rem">How many Stage / Off-stage programmes a student can join in total (Individual + Group combined).</div>
        <div class="grid2" style="margin-bottom:.75rem">
    ...
    adminContent.innerHTML = `
      <div class="card">
        ...
        <div class="row-between" style="margin-bottom:.4rem">
          ...
          <span class="muted" style="font-size:.65rem;font-family:'JetBrains Mono',monospace">${selStage}/${state.limits.maxStage} stage \u00b7 ${selOffStage}/${state.limits.maxOffStage} off-stage</span>
        </div>
        <div class="checkbox-list" id="checkList">
          ${eligible.length === 0 ? `<div class="muted" style="font-size:.72rem;font-style:italic">No programmes match this category/gender.</div>` :
            eligible.map((ev) => {
              ...
              const disabled = !checked && ((ev.type === "Group" && groupTeamFull(ev.id, studentForm.team)) || (stageType === "Stage" && selStage >= state.limits.maxStage) || (stageType === "Off-stage" && selOffStage >= state.limits.maxOffStage));
        ...
        <div class="checkbox-list" id="pCheckList">
          ${eligible.length === 0 ? `<div class="muted" style="font-size:.72rem;font-style:italic">No programmes match this category/gender.</div>` :
            eligible.map((ev) => {
              ...
              const stageType = ev.stageType || "Stage";
              const disabled = !checked && ((ev.type === "Group" && groupTeamFull(ev.id, student.team, student.id)) || (stageType === "Stage" && selStage >= state.limits.maxStage) || (stageType === "Off-stage" && selOffStage >= state.limits.maxOffStage));
  ...
  function renderGreenRoomSheetInline(wrap, sheets) {
    ...
    const sheetHtml = (s) => {
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:.25rem">
        <div>
          <div class="muted" style="font-size:.72rem">${escapeHtml(state.printHeaderName || state.hero.title)}</div>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.6rem;font-weight:700;font-size:.85rem">
        <div style="text-align:left">${escapeHtml(s.event.name)}</div>
        <div class="muted" style="font-weight:500;text-align:right;margin-left:auto;padding-left:.6rem">${escapeHtml(s.event.type || "Individual")} \u00b7 ${escapeHtml(s.event.category)}</div>
      </div>
      <div class="marks-table-wrap">
      <table class="schedule-print-table" style="table-layout:fixed">
  ...
  function renderExportTab() {
    ...
    document.querySelectorAll(".export-card").forEach((b) => b.addEventListener("click", () => {
      ...
      pickWrap.innerHTML = `
        <div class="card">
          ...
          <div style="position:relative">
            <input id="pickEventSearch" class="input" placeholder="Tap to search programme or category..." autocomplete="off" style="width:100%;padding-right:2.2rem" />
            <button type="button" id="pickEventDropdownArrow" class="dots-btn" style="position:absolute;right:.3rem;top:50%;transform:translateY(-50%);font-size:.9rem;padding:.2rem .4rem" title="Show all programmes">\u25BC</button>
      ...
      document.getElementById("pickEventDropdownArrow").addEventListener("click", (e) => {
        e.stopPropagation();
        // Always shows the FULL list (ignores whatever text is currently in
        // the box) \u2014 lets the admin pick a different programme after
        // already generating one, without having to clear the search text
        // first just to see the list again.
        showDropdown("");
        searchInput.focus();
      });
      document.addEventListener("click", (e) => {
        if (!e.target.closest("#pickEventSearch") && !e.target.closest("#pickEventDropdown") && !e.target.closest("#pickEventDropdownArrow")) dropdown.classList.add("hidden");
      });
  ...
  /* ---- Prize tab: pick a category, then a programme (in order), see its
     winners and tick them off as prizes are physically handed out. Makes
     stage-side prize distribution quick — no scrolling through every
     programme to find the next one that needs handing out. ---- */
  let prizeCategory = null;
  function renderPrizeTab() {
    if (prizeCategory === null || !state.categories.includes(prizeCategory)) {
      prizeCategory = state.categories[0] || null;
    }
    ...
    function prizeStatusFor(eventId) {
      const winners = getWinnersForEvent(eventId);
      if (!winners.length) return null; // no published result yet
      return { given: !!state.prizeGiven[eventId] };
    }

    function renderList() {
      const events = catEvents();
      const listWrap = document.getElementById("prizeListWrap");
      if (!listWrap) return;
      if (!events.length) {
        listWrap.innerHTML = `<div class="empty-note">No programmes in this category.</div>`;
        return;
      }
      listWrap.innerHTML = events.map((e) => {
        const stat = prizeStatusFor(e.id);
        let pillHtml;
        if (!stat) pillHtml = `<span class="status-pill status-pending">Not published</span>`;
        else if (stat.given) pillHtml = `<span class="status-pill status-published">\u2713 Prize given</span>`;
        else pillHtml = `<span class="status-pill status-submitted">Prize pending</span>`;
        return `
        <div class="history-row" data-id="${e.id}" style="cursor:pointer">
          <div class="history-row-main">
            <div class="history-row-title"><span class="muted">#${e._idx}</span> ${escapeHtml(e.name)}</div>
            <div style="margin-top:.35rem">${pillHtml}</div>
          </div>
          <span class="side-chevron" style="font-size:1.1rem;opacity:.5">&#8250;</span>
        </div>`;
      }).join("");
      listWrap.querySelectorAll("[data-id]").forEach((row) => row.addEventListener("click", () => openPrizeModal(row.dataset.id)));
    }

    adminContent.innerHTML = `
      <div class="card">
        <div class="card-title">\u{1F381} Prize Distribution</div>
        <div class="muted" style="font-size:.75rem;margin-bottom:.75rem">Pick a category to see its programmes in order, open one to view its winners, and tick it once the prize for that programme has been handed out.</div>
        <div class="field-label">Category</div>
        <select id="prizeCategorySelect" class="input" style="margin-bottom:.9rem">
          ${state.categories.map((c) => `<option value="${escapeAttr(c)}" ${c === prizeCategory ? "selected" : ""}>${escapeHtml(c)}</option>`).join("")}
        </select>
        <div id="prizeListWrap"></div>
      </div>`;

    document.getElementById("prizeCategorySelect").addEventListener("change", (e) => {
      prizeCategory = e.target.value;
      renderList();
    });
    renderList();
  }

  function openPrizeModal(eventId) {
    const event = state.events.find((e) => e.id === eventId);
    if (!event) return;
    ...
    function draw() {
      const winners = getWinnersForEvent(eventId);
      const given = !!state.prizeGiven[eventId];
      modalBody.innerHTML = `
        <div class="marks-modal">
          <div class="row-between" style="margin-bottom:.75rem;align-items:flex-start">
            <div>
              ...
              <div class="muted" style="font-size:.7rem;margin-top:.2rem">${escapeHtml(event.category)}</div>
            </div>
            <button class="dots-btn" id="pzClose" style="font-size:1.2rem">&times;</button>
          </div>
          ${!winners.length ? `<div class="empty-note">Result not published yet for this programme.</div>` : `
          <div class="checkbox-list" style="margin-bottom:.5rem">
            ${winners.map((w) => `
              <div class="checkbox-row" style="cursor:default">
                <span>${RANK_ICON[w.rank]} ${RANK_ORDINAL[w.rank]} \u2014 ${escapeHtml(resultDisplayName(event, w.student.name))}</span>
                <span style="margin-left:auto;font-size:.65rem" class="muted">${w.student.chestNo || ""}${w.team ? " \u00b7 " + escapeHtml(w.team.name) : ""}</span>
              </div>`).join("")}
          </div>
          <label class="checkbox-row ${given ? "checked" : ""}" style="border-width:2px">
            <input type="checkbox" id="pzGivenCheck" ${given ? "checked" : ""} />
            <span style="font-weight:700">\u{1F381} Prize given for this programme</span>
          </label>`}
          <div class="modal-actions" style="background:transparent;padding:.75rem 0 0;gap:.5rem">
            <button class="btn btn-ghost" id="pzClose2" style="flex:1">Close</button>
          </div>
        </div>`;
      document.getElementById("pzClose").addEventListener("click", closeTopScreen);
      document.getElementById("pzClose2").addEventListener("click", closeTopScreen);
      const givenCheck = document.getElementById("pzGivenCheck");
      if (givenCheck) givenCheck.addEventListener("change", () => {
        if (givenCheck.checked) state.prizeGiven[eventId] = true;
        else delete state.prizeGiven[eventId];
        persist();
        draw();
      });
    }
    ...
    draw();
    modalOverlay.classList.remove("hidden");
    pushScreen(() => { modalOverlay.classList.add("hidden"); modalBody.innerHTML = ""; renderPrizeTab(); });
  }
  ...
  function openResultViewModal(eventId) {
    ...
    modalBody.innerHTML = `
      <div class="marks-modal">
        ...
        <div style="overflow-x:auto">
          <table class="result-detail-table"><thead><tr>
            <th>Sl No</th><th>Chest #</th><th>Participant</th><th>Code Letter</th><th>Team</th><th>Prize</th><th>Points</th>
  ...
  function renderScheduleTab() {
    ...
    adminContent.innerHTML = `
      ...
      <button class="btn btn-primary" id="btnGenerateSchedule" style="width:100%;margin-top:1.25rem;padding:.75rem">\u{1F5A8} Print Schedule</button>
      <button class="btn btn-whatsapp" id="btnShareSchedule" style="width:100%;margin-top:.6rem;padding:.75rem">\u{1F4E4} Share Schedule (PDF)</button>`;
    ...
    document.getElementById("btnShareSchedule").addEventListener("click", shareScheduleSheet);
  }
      ...
      return `
        <div class="schedule-stage-block">
          ...
          <table class="schedule-print-table">
            <colgroup><col style="width:9%" /><col style="width:48%" /><col style="width:22%" /><col style="width:21%" /></colgroup>
  ...
  // Builds the same schedule as a standalone PDF (drawn directly with jsPDF,
  // matching the on-screen/print design: blue stage banners, red rule under
  // the title) and shares it via the device share sheet — WhatsApp etc.
  async function shareScheduleSheet() {
    if (!state.schedule.length) return showToast("Add at least one schedule entry first");
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF({ unit: "mm", format: "a4" });
    ...
    const groups = {};
    state.schedule.forEach((s) => { (groups[s.stage] = groups[s.stage] || []).push(s); });
    const stages = sortStageKeys(Object.keys(groups));

    const drawTitle = () => {
      doc.setFont("helvetica", "bold"); doc.setFontSize(16); doc.setTextColor(30, 58, 138);
      doc.text(state.hero.title, pageW / 2, y, { align: "center" }); y += 6;
      doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.setTextColor(220, 38, 38);
      doc.text("PROGRAM SCHEDULE", pageW / 2, y, { align: "center" }); y += 3;
    ...
    drawTitle();
    const colX = [margin, margin + 14, margin + 100, margin + 140];
    const colW = [14, 86, 40, pageW - margin * 2 - 140];
    const rowH = 7;

    stages.forEach((stage) => {
      ensureSpace(rowH * 2 + 4);
      doc.setFillColor(30, 58, 138); doc.rect(margin, y, pageW - margin * 2, rowH, "F");
      doc.setTextColor(255, 255, 255); doc.setFont("helvetica", "bold"); doc.setFontSize(9.5);
      doc.text(stage, margin + 3, y + rowH - 2.2);
      y += rowH;
      ...
      ["SL NO", "ITEM NAME", "CATEGORY", "TIME"].forEach((h, i) => doc.text(h, colX[i] + 1.5, y + rowH - 2.2));
      y += rowH;

      doc.setFont("helvetica", "normal");
      groups[stage].forEach((s, i) => {
        ensureSpace(rowH);
        const ev = state.events.find((e) => e.id === s.eventId);
        doc.rect(margin, y, pageW - margin * 2, rowH);
        const cells = [String(i + 1).padStart(2, "0"), ev ? ev.name : "", ev ? ev.category : "", formatTimeRange(s.start, s.end)];
        cells.forEach((c, ci) => {
          const text = doc.splitTextToSize(c, colW[ci] - 3);
          doc.text(text[0] || "", colX[ci] + 1.5, y + rowH - 2.2);
        });
        y += rowH;
      });
      y += 6;
    ...
    await shareBlobFile(blob, "Programme_Schedule.pdf", "application/pdf", `${state.hero.title} \u2014 Programme Schedule`);
  }
  ...
  function openPrintSheet(kind, eventId, skipHistoryLog) {
    ...
    if (kind === "Green Room Sign" && event.type === "Group") {
      body = `
        <div class="print-section-row"><b style="text-align:left">${escapeHtml(event.name.toUpperCase())}</b><span style="margin-left:auto;text-align:right">${event.type} \u00b7 ${escapeHtml(event.category.toUpperCase())}</span></div>
      ...
      body = `
        ...
        <table><thead><tr><th>Standing</th><th>Chest No</th><th>Candidate Name</th><th>Team</th><th>Points</th></tr></thead><tbody>
          ${list.map((r, i) => {
            const team = state.teams.find((t) => t.id === r.student.team);
            const points = i === 0 ? RANK_POINTS.first : i === 1 ? RANK_POINTS.second : i === 2 ? RANK_POINTS.third : null;
            return `<tr><td>${ORDINAL(i + 1)}</td><td>${r.student.chestNo}</td><td>${escapeHtml(resultDisplayName(event, r.student.name))}</td><td>${team ? escapeHtml(team.name) : ""}</td><td>${points ?? "-"}</td></tr>`;
          }).join("") || `<tr><td colspan="5" style="text-align:center;color:#999">No entries recorded yet.</td></tr>`}
        </tbody></table>
